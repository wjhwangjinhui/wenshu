...
# -*- coding: utf-8 -*-
"""
@Time    : 2018/12/15 14:47
@Author  : wangjh
@File    : peichang_list.py
@desc    : PyCharm
"""
import os
import random
...
def get_number(**kwargs):
    ...
    data = {'guid': guid}
    res = re_requests(url=url, data=data)
    if res:
        number = res.text
        return number
...
def get_vjx(**kwargs):
    with open(wenshujs, 'r', encoding='utf-8') as fp:
        js = fp.read()
    ect = execjs.compile(js)
    url = 'http://wenshu.court.gov.cn/List/List?sorttype=1&conditions=searchWord+4+AJLX++%E6%A1%88%E4%BB%B6%E7%B1%BB%E5%9E%8B:%E8%B5%94%E5%81%BF%E6%A1%88%E4%BB%B6'
    while True:
        try:
            headers = {
...
def get_data(**kwargs):
    ...
    headers = {
        ...
        # 'Host': 'wenshu.court.gov.cn',
        # 'Origin': 'http://wenshu.court.gov.cn',
        # 'Referer': 'http://wenshu.court.gov.cn/list/list/?sorttype=1&number=&guid={}&conditions=searchWord+4+AJLX++%E6%A1%88%E4%BB%B6%E7%B1%BB%E5%9E%8B:{}&conditions=searchWord+QWJS+++%E5%85%A8%E6%96%87%E6%A3%80%E7%B4%A2:{}'.format(guid, parse.quote('赔偿案件'), parse.quote(key))
